...
class VoxCADXBB : public QMainWindow
{
	Q_OBJECT

public:
	VoxCADXBB(QWidget *parent = 0, Qt::WFlags flags = 0);
	~VoxCADXBB();
	...
	void EnterVMMode (AppViewMode NewMode, bool Force = false);
...
public slots: //global slot repository for updating things across all windows/views...
	...
	void ImportVXA(){ForceViewMode(); QString FileName;  FastModeChanged(false); CheckNumVox(); ZoomExtAll(); SetWindowName(FileName); UpdateAllWins();}
	void SaveVXA(){QString NewFileName; SetWindowName(NewFileName);}
	...
	void UpdateAllWins() {EnterVMMode(VM_3DVIEW); PaletteDlg->UpdateUI(); WorkspaceDlg->IniUpdateUI(); ReqGLUpdateAll();}
...
//	void TensileTest(void);
	...
	void SetGLSelected(int NewSel=-1) {
		if (NewSel != CurGLSelected) {
			CurGLSelected=NewSel;
	...
	void LMouseDown(float X, float Y, float Z, bool IsCtrl) {if(CurViewMode==VM_EDITLAYER) MainObj.LMouseDown(Vec3D<>(X, Y, Z), IsCtrl);};
	void LMouseUp(float X, float Y, float Z) {if(CurViewMode==VM_EDITLAYER) MainObj.LMouseUp(Vec3D<>(X, Y, Z));};
	void LMouseDownMove(float X, float Y, float Z) {if(CurViewMode==VM_EDITLAYER) MainObj.LMouseDownMove(Vec3D<>(X, Y, Z));};
